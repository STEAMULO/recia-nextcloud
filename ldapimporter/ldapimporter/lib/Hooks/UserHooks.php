...
namespace OCA\LdapImporter\Hooks;
...
/**
 * Class LdapImporter_Hooks
 *
 * @package OCA\LdapImporter\Hooks
 *
 * @since 1.4.0
 */
class UserHooks
{
     ...
     * @param \OCA\LdapImporter\Service\UserService $userService
     * @param \OCA\LdapImporter\Service\AppService $appService
    ...
    public function preLogin($uid, $password)
    {
        ...
        if (!$this->appService->isCasInitialized()) {
            ...
            try {
                ...
                $this->loggingService->write(\OCA\LdapImporter\Service\LoggingService::FATAL, 'Fatal error with code: ' . $e->getCode() . ' and message: ' . $e->getMessage());
        ...
        if (\phpCAS::isAuthenticated() && !$this->userSession->isLoggedIn()) {
            ...
            if ($casUid === $uid) {
                ...
                if (boolval($this->config->getAppValue($this->appName, 'cas_autocreate'))) {
                    ...
                    $this->loggingService->write(\OCA\LdapImporter\Service\LoggingService::DEBUG, 'phpCas pre login hook triggered. User: ' . $uid);
                    ...
                    if (is_null($user)) {
                        ...
                        if (preg_match('/[^a-zA-Z0-9 _\.@\-]/', $uid)) {
                            ...
                            $this->loggingService->write(\OCA\LdapImporter\Service\LoggingService::DEBUG, 'Invalid username "' . $uid . '", allowed chars "a-zA-Z0-9" and "_.@-" ');
                            ...
                            if ($user instanceof \OCP\IUser) {
                                ...
                                $this->loggingService->write(\OCA\LdapImporter\Service\LoggingService::DEBUG, 'phpCAS created new user with UID: ' . $uid);
                            }
                        }
                    } else {
            ...
            $this->loggingService->write(\OCA\LdapImporter\Service\LoggingService::DEBUG, 'phpCas pre login hook NOT triggered. User: ' . $uid);
        }
    ...
    public function postLogin($uid, $password)
    {
        ...
        if (\phpCAS::isAuthenticated() && $this->userSession->isLoggedIn()) {
            ...
            if (boolval($this->config->getAppValue($this->appName, 'cas_update_user_data'))) {
                ...
                $this->loggingService->write(\OCA\LdapImporter\Service\LoggingService::DEBUG, 'phpCas post login hook triggered. User: ' . $uid);
                ...
                if ($casUid === $uid) {
                    ...
                    // parameters
                    $attributes = array();
                    $this->loggingService->write(\OCA\LdapImporter\Service\LoggingService::DEBUG, 'Attributes for the user: ' . $uid . ' => ' . $casAttributesString);
                    ...
                    if (array_key_exists($groupMapping, $casAttributes)) {
                        ...
                        $this->loggingService->write(\OCA\LdapImporter\Service\LoggingService::DEBUG, 'Using default group "' . $defaultGroup . '" for the user: ' . $uid);
                    }
                    ...
                    #$this->loggingService->write(\OCA\LdapImporter\Service\LoggingService::DEBUG, 'phpCas quota-mapping-contents: '.$userQuotaMapping);
                ...
                $this->loggingService->write(\OCA\LdapImporter\Service\LoggingService::DEBUG, 'phpCas post login hook finished.');
            }
        } else {

            $this->loggingService->write(\OCA\LdapImporter\Service\LoggingService::DEBUG, 'phpCas post login hook NOT triggered. User: ' . $uid);
        }
    ...
    public function postLogout()
    {
        ...
        $this->loggingService->write(\OCA\LdapImporter\Service\LoggingService::DEBUG, 'Logout hook triggered.');

        if (!boolval($this->config->getAppValue($this->appName, 'cas_disable_logout'))) {

            $this->loggingService->write(\OCA\LdapImporter\Service\LoggingService::DEBUG, 'phpCAS logging out.');
            ...
            $this->loggingService->write(\OCA\LdapImporter\Service\LoggingService::DEBUG, 'phpCAS not logging out, because CAS logout was disabled.');
        }
