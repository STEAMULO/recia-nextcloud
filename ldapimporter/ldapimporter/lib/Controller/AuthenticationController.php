...
class AuthenticationController extends Controller
{
    ...
    {
        $tmp = $this->config->getAppValue($this->appName, 'cas_debug_file', '');
        $tmp2 = $this->config->getAppValue($this->appName, 'cas_php_cas_path', '');
        ...
        if (isset($_COOKIE['ldap_importer_redirect_url'])) {

            $url = urldecode($_COOKIE['ldap_importer_redirect_url']);
        ...
        $this->loggingService->write(\OCA\LdapImporter\Service\LoggingService::DEBUG, 'The Redirect URL Parameter in Login Action was: ' . $location);
        ...
        if (!$this->userService->isLoggedIn()) {
            ...
            try {
                ...
                if (\phpCAS::isAuthenticated()) {
                    ...
                    $this->loggingService->write(\OCA\LdapImporter\Service\LoggingService::DEBUG, "phpCAS user " . $userName . " has been authenticated.");

                    $isLoggedIn = $this->userService->login($this->request, $userName);

                    if ($isLoggedIn) {

                        $this->loggingService->write(\OCA\LdapImporter\Service\LoggingService::DEBUG, "phpCAS user has been authenticated against owncloud.");
                        ...
                        $this->loggingService->write(\OCA\LdapImporter\Service\LoggingService::DEBUG, "phpCAS user has not been authenticated against owncloud.");

                        return $this->casError(null, \OCP\AppFramework\Http::STATUS_FORBIDDEN);
                    }
                } else { # Not authenticated against CAS

                    $this->loggingService->write(\OCA\LdapImporter\Service\LoggingService::DEBUG, "phpCAS user is not authenticated, redirect to CAS server.");
            ...
            $this->loggingService->write(\OCA\LdapImporter\Service\LoggingService::DEBUG, "phpCAS user is already authenticated against owncloud.");
    ...
    private function casError(\Exception $exception = NULL, $additionalErrorCode = 0)
    {
        ...
        if ($additionalErrorCode !== 0) {
            ...
            if ($additionalErrorCode === \OCP\AppFramework\Http::STATUS_FORBIDDEN) {
                ...
                if (boolval($this->config->getAppValue('ldap_importer', 'cas_ecas_attributeparserenabled', false))) {
